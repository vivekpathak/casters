...
def _generate_view_with_testcase(viewdirname) :
    ...
    open( viewdirname + "/tests/caster_harness.js", "w").write(
        '''// auto-generated by caster ; do not edit - improve the script on github instead
var keys = [] ;
var values = [] ;
function emit(k, v) {
  keys.push(k)  ;
...
try {
   ...
   print ("note : did not find reduce function" )  ;
}
mapcode = read("../map.js");
eval(" map = " + mapcode ) ;
if(reducecode) {
...
''')
    open( viewdirname + "/tests/testExample.js", "w").write(
        '''// sample generated by caster
// modify as needed
// see https://developer.mozilla.org/en-US/docs/SpiderMonkey/Creating_JavaScript_tests
load("caster_harness.js") ;   // creates fns & variables : map() , reduce() , keys , values


var doc = { "_id" : "ABCD" , "dummy" : "DUMMY" } ;
...
''')
    open( viewdirname + "/map.js" , "w" ).write(
        '''// sample generated by caster
// modify as needed
function(doc) {
  emit(doc._id, doc);
